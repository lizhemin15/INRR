...
class siren(basic_demo):
    def __init__(self,para=[2,100,100,1],reg=None,img=None,opt_type='Adam',omega=30.,drop_out=[0,0,0,0,0],ynet=None,ysample={}):
        self.net = net.siren(params=para,img=img,lr=1e-4,opt_type=opt_type,omega=omega,drop_out=drop_out,ynet=ynet,ysample=ysample)
        self.reg = reg
        self.loss_dict={'loss_fid':[],'loss_all':[],'nmae_test':[]}
        for reg_now in self.reg:
            self.loss_dict['loss_'+reg_now.type] = []
...
class att(basic_demo):
    def __init__(self,x_train=None,y_train=None,x_test=None,dim_k=10,mask=None,reg=None,performer_if=False):
